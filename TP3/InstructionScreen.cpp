...
Scene::scenes InstructionScreen::run()
{
	while (isRunning)
	{
		getInputs();
...
bool InstructionScreen::init(RenderWindow * const window)
{
	if (!ScreenImageT.loadFromFile("Sprites\\tp3_instructions.png"))
	{
		return false;
	}
...
void InstructionScreen::getInputs()
{
...
void InstructionScreen::transition(const Scene::scenes & scene)
{
	isRunning = false;
